...
% ==========================================
% AUKSINĖ TAISYKLĖ IR PAVYZDYS
% ==========================================
\begin{center}
\fbox{
\begin{minipage}{0.9\textwidth}
\textbf{Auksinė taisyklė: Išsibandykite patys („Pažaiskite“)}

Pamatę uždavinį apie procesą, nesistenkite iš karto atspėti invarianto. Paimkite popieriaus lapą ir atlikite kelis ėjimus ranka! Sumažinkite pradinius skaičius (jei duota 100, paimkite 4) ir pažiūrėkite, kas nutinka su lyginumu, sumomis ar skirtumais. Stebint savo paties atliktus ėjimus, pamatyti tai, kas \textbf{nesikeičia}, tampa kur kas lengviau.
\end{minipage}
}
\end{center}

\begin{example}[Auksinės taisyklės demonstravimas]
\textbf{Uždavinys:} Lentoje surašyti skaičiai $1, 2, \ldots, 2026$. Vienu ėjimu leidžiama nutrinti bet kokius du skaičius $a$ ir $b$, o vietoje jų parašyti $|a-b|$. Ar gali paskutinis lentoje likęs skaičius būti nulis?
\end{example}

\begin{solution}
\textbf{Sprendimas:}\\
Užuot iškart žiūrėję į 2026 skaičius, paimkime tik keturis: $1, 2, 3, 4$. Stebėkime ne tik pačius skaičius, bet ir jų \textbf{sumą}.
\begin{itemize}
    \item \textbf{Pradinė būsena:} $1, 2, 3, 4$. Suma yra $10$ (\textbf{lyginis} skaičius).
    \item \textbf{1 ėjimas:} Nutrinkime $2$ ir $3$, parašykime $|2-3| = 1$. Lentoje lieka $1, 4, 1$. Suma dabar $1+4+1 = 6$ (\textbf{lyginis}!).
    \item \textbf{2 ėjimas:} Nutrinkime $4$ ir $1$, parašykime $|4-1| = 3$. Lentoje lieka $1, 3$. Suma dabar $1+3 = 4$ (\textbf{lyginis}!).
    \item \textbf{3 ėjimas:} Nutrinkime $1$ ir $3$, parašykime $|1-3| = 2$. Lentoje liko skaičius $2$. 
\end{itemize}

\textbf{Atradimas:} Atliekant ėjimus, skaičių suma keitėsi ($10 \to 6 \to 4 \to 2$), bet jos \textbf{lyginumas} niekada nesikeitė – ji visada liko lyginė! 
Kodėl? Trinant skaičius $a, b$ iš bendros sumos atimama $(a+b)$, o pridedamas skirtumas $(a-b)$. Matematikoje skaičių sumos $a+b$ ir skirtumo $a-b$ lyginumas \textbf{visada sutampa}. Vadinasi, bendros sumos lyginumas viso proceso metu nesikeičia.

\textbf{Pritaikymas originaliam uždaviniui:} 
Suskaičiuokime pradinę visų 2026 skaičių sumą:
\[ 1+2+3+\ldots+2026 = \frac{2026 \cdot 2027}{2} = 1013 \cdot 2027 \]
Dauginame du nelyginius skaičius, todėl pradinė suma yra \textbf{nelyginė}.
Remiantis mūsų atrastu invariantu, skaičių suma po bet kokio ėjimų skaičiaus privalo išlikti nelyginė. Jei lentoje liktų nulis (kuris yra lyginis skaičius), suma taptų lyginė. Tai neįmanoma! \hfill $\square$
\end{solution}
...
\begin{solution}
\textbf{Sprendimas:} \\
\textit{(Patarimas: Prieš skaitant toliau, pabandykite ranka sudėlioti ženklus tik skaičiams 1, 2, 3 ir paskaičiuoti sumas. Pamatysite, kad jos visada lyginės!)}
...
\begin{solution}
\textbf{Sprendimas:}\\
\textit{(Patarimas: Paimkite 2 pliusus ir 3 minusus. Trinkite juos pagal taisyklę. Koks ženklas visada lieka pabaigoje?)}
...
\begin{solution}
\textbf{Sprendimas:}\\
\textit{(Patarimas: Pabandykite sumaišyti tik skaičius 1, 2, 3 ir paskaičiuoti sandaugą $(x_1-1)(x_2-2)(x_3-3)$. Juk nors vienas narys $(x_k - k)$ visada išeina lyginis!)}
...
\begin{solution}
\textbf{Sprendimas:}
\textit{(Patarimas: Nusipieškite $3 \times 3$ lentą ir pažiūrėkite, kaip keičiasi langelių spalvos. Langeliai turi būti dviejų spalvų, o žirgas – spalvų keitimo mašina!)}
...
\begin{solution}
\textbf{Sprendimas:}\\
\textit{(Patarimas: Paimkite mažą 2x2 lentą ir iškirpkite du priešingus kampus. Ar galite uždėti bent vieną domino? Kokių spalvų langeliai liko?)}
...
\begin{solution}
\textbf{Sprendimas:}\\
\textit{(Patarimas: Nupieškite ant langinio popieriaus T formos figūrą ir „pavaikščiokite“ su ja kaip šachmatais nuspalvinta lenta. Kiek juodų langelių ji visada uždengia?)}
...
% 3 STRATEGIJA: GRAFAI
% ==========================================
\section{3 strategija: Taškai ir ryšiai (Grafų teorijos elementai)}

Kai olimpiadinėje matematikoje išgirsite terminą \textbf{„grafas“} ar „grafų teorija“, neišsigąskite. Tai neturi visiškai nieko bendro su funkcijų grafikais ($y = x^2$) ar koordinačių ašimis, kuriuos braižote per pamokas. 
Olimpiadinis grafas yra pats paprasčiausias piešinys: tai tiesiog \textbf{taškai} (kurie reiškia žmones, miestus, salas, komandas) ir tuos taškus jungiančios \textbf{linijos} (kurios reiškia draugystes, kelius, tiltus, rungtynes). Štai ir visa teorija!
Ši strategija (taškų ir linijų braižymas) naudojama, kai uždavinys kalba apie ryšius tarp objektų. Čia dažniausiai veikia du galingi olimpiadininko ginklai: ryšių skaičiavimas („Rankų paspaudimų lema“) ir uždarų ratų (ciklų) stebėjimas. 

\textbf{Pagrindinė ryšių taisyklė:} kiekvienas ryšys (tiltas, draugystė, virvė) sujungia du taškus, todėl jis visada turi DU galus.
\textbf{Pagrindinė ryšių taisyklė:} kiekvienas ryšys (tiltas, draugystė, virvė) sujungia du objektus, todėl jis visada turi DU galus.
...
\begin{solution}
\textbf{Sprendimas:} 
\textit{*(Patarimas: Pabandykite nupiešti tris draugus (A, B, C) linijomis. Jei kiekvienas turi po 3 draugus, ar pavyks tai nupiešti? Kur trūks ryšio galo?)}*
...
\begin{solution}
\textbf{Sprendimas:}\\
\textit{(Patarimas: Paimkite pieštuką ir pabandykite bent du kartus apibrėžti nupieštą grafą. Atkreipkite dėmesį, kad kaskart pravažiuodami tarpinę salą (ne pradžios, ne pabaigos), Jūs sunaudojate du tiltus (vieną atvykimui, kitą išvykimui).)}
...
\noindent \textbf{Nelyginiai ciklai grafuose} \\
Išmokę stebėti atskirus ryšius, olimpiadininkai atkreipia dėmesį į dar vieną grafų struktūrą – uždarus ciklus (ratus). Čia sugrįžta mūsų antrojoje strategijoje išmokta būsenų kaitaliojimo (dažymo) idėja, tik dabar ji pritaikoma sujungtiems taškams.
...
\begin{solution}
\textbf{Sprendimas:}
\textit{*(Patarimas: Nusipieškite tris krumpliaračius sujungtus ratu ir pabandykite nupiešti sukimosi rodykles. Kas atsitinka su trečiuoju?)}*

Krumpliaračių mechanika remiasi vienu dėsniu: jei vienas krumpliaratis sukasi pagal laikrodžio rodyklę (žymėkime +), su juo besiliečiantis privalo suktis prieš laikrodžio rodyklę (žymėkime –). Matome tą pačią būsenų kaitaliojimosi taisyklę, kaip ir šachmatų lentoje!

Kadangi ratas uždaras, krumpliaračių kryptys privalo kaitaliotis: +, –, +, –, +, ... 
Tačiau mes turime \textbf{nelyginį} (11) krumpliaračių skaičių. Pirmasis (+) ir 11-asis (+) turės suktis ta pačia kryptimi. Bet jie liečiasi vienas su kitu! Du besiliečiantys ir ta pačia kryptimi besisukantys krumpliaračiai sistemą akimirksniu užblokuoja. (Ši taisyklė grafų teorijoje skamba taip: nelyginio ilgio ciklai negali būti nuspalvinti dvejomis spalvomis). \hfill $\square$
\end{solution}
\end{minipage}
\hfill
\begin{minipage}{0.35\textwidth}
...
% ==========================================
\newpage
\section{Uždaviniai savarankiškam sprendimui (43 iššūkiai)}

Šie uždaviniai surikiuoti pagal sunkumą (nuo apšilimo iki ekspertinio lygio). Jūsų tikslas nėra tik pasakyti „Taip“ arba „Ne“. Tikslas – atrasti invariantą ir pateikti \textbf{griežtą matematinį įrodymą}.
...
\begin{enumerate}
    ...
    \item Lentoje surašyti skaičiai $1, 2, 3, \ldots, 2025$. Vienu ėjimu leidžiama nutrinti bet kokius du skaičius $a$ ir $b$, o vietoje jų parašyti $a+b+2$. Ar gali paskutinis lentoje likęs skaičius būti lyginis?
    ...
    \item Dvidešimt penkios šaškės išdėliotos $25 \times 25$ dydžio šachmatų lentoje taip, kad jų išdėstymas yra simetriškas vienos iš lentos įstrižainių atžvilgiu. Įrodykite, kad bent viena šaškė stovi ant šios įstrižainės.
    \item Duotas iškilusis 101-kampis, turintis simetrijos ašį. Įrodykite, kad simetrijos ašis eina per vieną iš jo viršūnių.
    ...
    \item Ar galima nubrėžti uždarą laužtę iš $9$ atkarpų, kurios kiekviena atkarpa kerta lygiai vieną iš kitų atkarpų?
    \item Visi domino rinkinio kauliukai yra sudėti į vientisą grandinę (laikantis taisyklių). Jei viename grandinės gale yra $5$ taškeliai, kas yra kitame gale?
    \item Sraigė griaužia $3 \times 3 \times 3$ sūrio kubą, sudarytą iš $27$ mažų kubelių. Ji pradeda nuo vieno kampinio kubelio ir kiekvieną kartą pergraužia sieną pereidama į gretimą kubelį. Ar gali ji suvalgyti visus kubelius ir kelionę baigti pačiame centriniame kubelyje?
    ...
    \item Sraigė šliaužia plokštuma pastoviu greičiu, kas $15$ minučių pasukdama stačiu kampu (90 laipsnių). Įrodykite, kad sraigė gali grįžti į pradinį tašką tik praėjus sveikam valandų skaičiui.
    \item Iš domino kauliukų rinkinio pašalinami visi tie, kurie turi bent vieną tuščią pusę (be taškelių). Ar galima likusius domino kauliukus sudėti į vientisą grandinę?
    \item Būryje yra $100$ kareivių, o kiekvieną vakarą trys iš jų eina budėti. Ar gali atsitikti taip, kad po tam tikro laiko kiekvienas kareivis bus budėjęs su kiekvienu kitu kareiviu lygiai po vieną kartą?
    \item Aikštelėje guli trys ledo rituliai A, B ir C. Ledo ritulininkas muša vieną iš jų taip, kad šis pračiuožia tarp kitų dviejų. Jis tai padaro $25$ kartus. Ar gali jis grąžinti tris ritulius į jų pradines vietas?
    \item Trys žiogai žaidžia šokinėjimo žaidimą išilgai tiesės. Kiekvieno ėjimo metu vienas žiogas peršoka per kitą žiogą, bet ne per du iš karto. Ar žiogai gali grįžti į savo pradines pozicijas po $1991$ šuolio?
    \item Ar galima iškilųjį 13-kampį padalinti į lygiagretėnius?
    \item Uždarą laužtę sudaro $11$ atkarpų. Ar gali viena tiesė, neinanti per jokią laužtės viršūnę, kirsti kiekvieną iš jos atkarpų?
    \item Parenkamas 17 skaitmenų skaičius ir jo skaitmenys perrašomi atvirkščia tvarka, gaunant naują skaičių. Šie du skaičiai sudedami. Įrodykite, kad jų sumoje yra bent vienas lyginis skaitmuo.
    \item Ar galima sudaryti „magiškąjį kvadratą“ iš pirmųjų $36$ pirminių skaičių? (Magiškasis kvadratas čia yra $6 \times 6$ lentelė, kurioje eilučių, stulpelių ir įstrižainių sumos yra vienodos).
    \item Lentelės $25 \times 25$ langeliuose bet kaip surašyti skaičiai $+1$ ir $-1$. Tegu $e_1, e_2, \ldots, e_{25}$ yra kiekvienos eilutės skaičių sandaugos, o $s_1, s_2, \ldots, s_{25}$ – kiekvieno stulpelio skaičių sandaugos. Ar gali visų šių 50-ies atsakymų suma būti lygi $0$?
