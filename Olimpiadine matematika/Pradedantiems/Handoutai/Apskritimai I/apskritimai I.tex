...
\newpage
\newpage
\begin{example}[4. Įbrėžtos trapecijos taisyklė]
Apskritime nubrėžtos dvi lygiagrečios stygos $AB \parallel CD$. Įrodykite, kad gautas keturkampis $ABCD$ visada yra lygiašonė trapecija (t.y., jo šoninės kraštinės $AD = BC$).
\end{example}
...
\begin{tikzpicture}[scale=1.4]
    ...
    \draw[thick, -stealth] ($(A)!0.5!(B) - (0.1,0)$) -- ($(A)!0.5!(B) + (0.1,0)$);
    \draw[thick, -stealth] ($(D)!0.5!(C) - (0.1,0)$) -- ($(D)!0.5!(C) + (0.1,0)$);
...
\textbf{Sprendimas:}\\
Sujungsime Geometrijos I (Euklido bėgiai) ir Geometrijos II (Lankai) žinias!

\textbf{1 Žingsnis (Pagalbinė linija ir vidaus priešiniai kampai):} Keturkampis $ABCD$ turi dvi lygiagrečias kraštines ($AB \parallel CD$), todėl tai – trapecija. Nubrėžkime jos įstrižainę $AC$. Remdamiesi I-ajame handoute išmokta lygiagrečių tiesių savybe, iškart matome vidaus priešinius kampus:
\[
...
\textbf{2 Žingsnis (Kampų vertimas į lankus):} 
Įbrėžtinis kampas $\angle BAC = \alpha$ remiasi į lanką $BC$. Vadinasi, lankas dvigubai didesnis: $\wideparen{BC} = 2\alpha$.
Lygiai taip pat, įbrėžtinis kampas $\angle ACD = \alpha$ remiasi į lanką $AD$. Vadinasi, $\wideparen{AD} = 2\alpha$.
...
\textbf{3 Žingsnis (Išvada):} Dabar patys įrodykite, kad $AD = BC$. Pažymėkite apskritimo centrą $O$ ir sujunkite centro tašką $O$ su taškais $A$, $B$, $C$ ir $D$. Kadangi visi šie taškai yra ant to paties apskritimo, turime $OA = OB = OC = OD$. Be to, vienodi lankai $\wideparen{AD}$ ir $\wideparen{BC}$ atitinka vienodus centrinius kampus, todėl $\angle AOD = \angle BOC$. Prisiminkite trikampių lygumo požymį kraštinė–kampas–kraštinė ir įrodykite, kad trikampiai $AOD$ ir $BOC$ yra lygūs. Iš čia gausite:
\[
AD = BC.
\]
Įrodyta! Olimpiadose galite naudoti šį faktą iškart: \[\textbf{jei trapecija įbrėžta į apskritimą, ji privalo būti lygiašonė}.\] \hfill $\square$
\newpage
...
\item Smailusis trikampis $ABC$ įbrėžtas į apskritimą. Taškas $M$ yra lanko $AB$ vidurys, o taškas $N$ – lanko $AC$ vidurys. Styga $MN$ kerta kraštines $AB$ ir $AC$ atitinkamai taškuose $X$ ir $Y$. Įrodykite, kad atsiradęs mažasis trikampis $AXY$ yra lygiašonis ($AX = AY$). 
\end{enumerate}
